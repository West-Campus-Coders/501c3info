...
export default function DemoPage() {
    ...
    const fetchData = () => {
     if(city == ''){
        axios.get(`https://501c3api.online/nonprofit/${usState}`).then((res) => {
            console.log(res);
            setData(res.data);
        }).catch((err) => {
            console.log(err);
     ...
     if(city != ''){
        axios.get(`https://501c3api.online/nonprofit/${usState}/${city}`).then((res) => {
            console.log(res);
            setData(res.data);
        }).catch((err) => {
            console.log(err);
    ...
    const columns = useMemo<MRT_ColumnDef<Nonprofits>[]>(
        () => [
            {
                accessorKey: 'NAME',
    ...
    const csvConfig = mkConfig({
        ...
        const handleExportRows = (rows: MRT_Row<Nonprofits>[]) => {
